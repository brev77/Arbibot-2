...
- **`on_chain_transactions` writer (PLAN9 P9-2, delivery 2026-08-03):** `OnChainTransactionService.persistWithOutcome()` — единственный writer, вызывается из `LegsService.completeMarkSent()` **атомарно** с переходом `execution_legs` `submitting → sent` и outbox-emit (`DexOutboxEventsService.emitConfirmed` / `emitFailed`). До PLAN9 таблица была пустой (0 writers) — это закрытый аудита PLAN9 блокер. Заполняет `tx_hash`, `status` (`confirmed`/`failed`/`reverted`), `input_data` (BYTEA), `gas_used`, `block_number`, `revert_reason` при наличии.
- **Live-completion feedback (PLAN10 P10-FB):** settlement-relay отправляет HTTP callback `POST /opportunities/:planId/live-completed` в opportunity-service — **НЕ** через outbox read. Причина: `outbox_events.processed_at` — общая колонка для всех allowlist'ов; при пересекающихся allowlist (in-DB relay + Kafka bridge) возникает race, кто первым отметит processed. Для feedback-канала live-execution выбран прямой HTTP callback (at-least-once через retry), не разделённый outbox-read.
