...
| 57 | `FUNC-PROBE-COVERAGE` | FUNC (REL) | paper-check | new | 5 | 3 | 15 | review | PLAN14 — полная вселенная + сырьевой тир |
...
**Аналитика сырого ряда (расширение решением оператора №6, после миграции 059):** по
...
| **Корневые файлы** | `tools/seed-registry-defillama.mjs`, `probe-discovery.mjs` (`FACTORIES`, probing, MC3-чтения), `probe-dry-run.mjs` (вселенная Phase 2 → trigger-driven), миграция `059` |
...
2. **Сырьевой тир.** Каждые `raw.intervalCycles` (дефолт 3) — Multicall3 `aggregate`-батчи
   ...
   Результат — таблица миграции 059:
...
- [~] Coverage-аудит live: большинство venue 98–100%; **объяснённый разрыв**: slipstream
      38% (velodrome) / 66% (aerodrome) — фабричный ts-пробинг покрывает не все тир-спейсинги;
      newborn-сэмплинг докрывает со временем; follow-up — полная ts-лестница + вторые деплои фабрик.
- [x] Ре-сит `SEED_TVL_MIN=0` залит (1303 кандидата; Base UniV3 826 пулов, registry ~1.6K).
- [x] Сырьевой тир live: `dry_run_raw_token_prices` пишется (53 токена/проход, 335 групп);
      MC3-чтения с бисекцией батча (один плохой пул больше не валит 25).
- [x] Триггер live: fee-adjusted спред >10 bps → Phase 2 trigger-driven **12–14/343 токенов**
      (было 343); цикл сжался до **145–152 с** (было 270–310); фолбэк на eligible при провале сырья.
- [x] Fee-adjusted: юнит-фикстура «1% vs 0.05%» — фантомных триггеров нет (12/12 тестов).
- [ ] После ≥48 ч сырья: калибровка triggerBps по распределению 10/15/25 bps (по плану).
- [ ] Retention-тиринг: код live, первая фактическая свёртка — через 48 ч сырья.
- [x] Гейт валидности сырья (ненулевые резервы, sane-диапазон цены) — в тестах.
- [x] Sushi: **55 пулов** Arb (root-cause: Base-фабрика EOA + канонический адрес = чужой код
      on Base, OP отсутствует в дампе — всё верифицировано и задокументировано в сидере).
- [x] OP sushi: root-cause отчёт (в дампе отсутствует, фабрика не верифицирована).
- [x] Newborn-пробинг live: +92 (OP) / +136 (Base) / +55 (Arb sushi) за первый цикл.
...
- [x] Событийный триггер на тестовом свопе (фикстура лога) → квота вне цикла,
      `metadata.trigger='event'`.
      *Верифицировано 2026-08-18 на Aéza (`tools/probe-event-smoke.mjs --fixture`):
      WSTETH-фикстура → 8 cc-obs строк `event-<uuid>` + 3 run_stats `source='event'`,
      квота-пасс 1.9 c; canonical-негатив ($100K WBTC) → 0 квот. Спек и факты —
      [`plan-probe-event-triggers-2026-08-18.md`](plan-probe-event-triggers-2026-08-18.md).*
- [x] Кап event-квот/час соблюдается (учёт в run_stats).
      *Юнит-тесты hourlyCapAllows/cooldownAllows (18/18); телеметрия run_stats
      source='event' пишется (rpc-дельта, batch-ms).*
- [ ] Суммарный RPC в бюджете: guard #56 не срабатывает чаще фонового уровня.
      *(живая телеметрия первых суток — хвост review).*
- [ ] Латентность «событие → квота» ≤ 60 c на живом стенде (замер по логам).
      *(механика: квота 1.9 c + poll 30 c ≈ 20 c типично; живое подтверждение по
      первым реальным триггерам — хвост review).*
...
#52 exec_pp ──→ #53 окна ──→ #56 MC3/hot-cold ──→ #57 raw-тир ──→ #58 event-триггеры
                                        │                │
                                        └────────────────┴──→ #54 FilterLab (полный: после 059 + 48–72 ч данных)
