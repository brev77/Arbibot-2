...
- **`evaluateTrade()` wired in:** every DEX adapter (`uniswap-v2`, `uniswap-v3`, `sushiswap-v2`, `pancakeswap-v2`, `biswap-v2`) calls `dexRiskPolicy.evaluateTrade({...})` before `selectWallet`, throwing on `deny`. The single insertion point is the adapter's swap entry; bridge legs are gated by the kill-switch + their own finality (L5), not `evaluateTrade`.
- **Daily volume persisted:** the in-memory `dailyVolume` Map moves to a Postgres table (`dex_daily_volume` keyed by `chain_id, trade_date`) updated inside the same tx as the leg transition; survives restart. New migration in `D4-B-2`.
