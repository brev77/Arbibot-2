...
| Когда | Что | Заметки |
|--------|-----|--------|
| Latent bug: `PaperPromotionService.approve` | Фикс: метод принимает precondition `status === 'queued'`, но `assertTransition('queued','promoted')` бросит `ConflictException` — фактически approve работает только на `under_review`. Не блокирует PAD (operator-approved candidates уже `under_review`), но контракт вводит в заблуждение. | Файл: `apps/paper-trading-service/src/paper/paper-promotion.service.ts:205-246`. Найдено при аудите PAD. |
...
| Перед live с реальным капиталом | **Drill: Reconciliation P0** | Искусственно создать mismatch (update portfolio.manual_override); reconciliation service детектирует < 15m; operator проходит P0 procedure | **Симулятор готов (P7-4):** `npm run drill:2` ([`tools/drill-2-reconciliation.mjs`](../tools/drill-2-reconciliation.mjs)) + [`docs/drill-2-reconciliation.md`](drill-2-reconciliation.md); [`docs/reconciliation-p0-procedures.md`](reconciliation-p0-procedures.md) |
| Перед live | **Drill: Disaster recovery (DB restore)** | `pg_dump` snapshot → drop database → restore → verify migrations → smoke test (paper trading). RTO/PO измеряются | **Симулятор готов (P7-5):** `npm run drill:3` ([`tools/drill-3-disaster-recovery.mjs`](../tools/drill-3-disaster-recovery.mjs)) + [`docs/drill-3-disaster-recovery.md`](drill-3-disaster-recovery.md) (работает с отдельной test-БД, safety-abort при совпадении dbname); [`docs/disaster-recovery-plan.md`](disaster-recovery-plan.md) |
...
|----|------|-----|-------|------|----------|--------|
| **C1** | ~~Bridge fee estimation — заглушки (TODO в across + stargate). Реальные деньги при live bridge.~~ → **✅ RESOLVED (2026-08-01, code-first verify для PLAN7):** `estimateBridgeFee` реализован в обоих адаптерах — `across-bridge.adapter.ts:398` (suggested-fees API + gas via price oracle), `stargate-bridge.adapter.ts:508` (on-chain `quoteLayerZeroFee` + gas). Закрыто коммитом `58848ec feat(execution): pre-trade cost estimation — gas + slippage + pool/bridge fees`. TODO.md устарел; пример принципа P1 из [`docs/roadmap-vectors.md`](roadmap-vectors.md). | — | — | | ✅ resolved |
...
| **C3** | ~~`audit-service` — 0 unit-тестов (compliance-trail непроверен).~~ → **✅ RESOLVED (2026-08-01, code-first verify для PLAN7):** тесты существуют — `apps/audit-service/src/audit/audit.controller.spec.ts` + `audit.service.spec.ts`. TODO.md устарел. | — | — | | ✅ resolved |

### 🟠 High (блокеры live-деплоя; для paper — проверить)

| ID | Риск | Где | paper | live | Владелец | Статус |
|----|------|-----|-------|------|----------|--------|
| **H1** | ~~Hardcoded salt в `KeyVaultService`~~ → **✅ RESOLVED (2026-08-01, P7-6):** salt вынесен в env `VAULT_MASTER_KEY_SALT` (`key-vault.service.ts:84`), с backward-compatible fallback на историческую константу для dev/тестов. `tools/validate-env.sh` требует явную установку в prod (fail-closed) и отвергает историческое значение. `PRIVATE_KEY_ENCRYPTION_KEY` тоже добавлен в validator (L3). ADR `docs/adr-vault-salt.md`. 4 новых теста (89/89 ✅). | — | — | | ✅ resolved (P7-6) |
| **H2** | ~~API key comparison **timing-unsafe** в `HermesAuthGuard` (не constant-time).~~ → **✅ RESOLVED (2026-08-01, code-first verify для PLAN7):** `apps/hermes-gateway/src/hermes/hermes-auth.guard.ts:1,38-57` использует `crypto.timingSafeEqual` с защитой от length-leak (dummy-сравнение при разной длине, без early-exit). | — | — | | ✅ resolved |
| **H3** | ~~`config-service/panic.service` без unit-тестов (panic button — деструктивный путь).~~ → **✅ RESOLVED (2026-08-01, code-first verify для PLAN7):** тесты существуют — `apps/config-service/src/config/panic.service.spec.ts` + `panic.controller.spec.ts`. | — | — | | ✅ resolved |
| **H4** | ~~`token-approve.service` без unit-тестов (ERC-20 approve перед swap — ошибки = потеря средств).~~ → **✅ RESOLVED (2026-08-01, code-first verify для PLAN7):** тест существует — `apps/execution-orchestrator/src/execution/token/token-approve.service.spec.ts`. | — | — | | ✅ resolved |
| **H5** | ~~`paper-capital.service` без unit-тестов (резервирование виртуальных средств).~~ → **✅ RESOLVED (2026-08-01, code-first verify P7-pre):** spec существует — `apps/paper-trading-service/src/paper/paper-capital.service.spec.ts`. TODO.md устарел. | — | — | | ✅ resolved |
| **H6** | ~~**Структурный пробел «два домена инцидентов»**~~ → **✅ RESOLVED (2026-08-01, P7-7):** pull-pipeline реализован — gateway `GET /hermes/v1/alerts` (read-through → reconciliation `/alerts/incidents`), MCP tool `list_alertmanager_incidents` (registered, всего 25 tools), cron `alert_watch` (интервал `${ALERT_WATCH_INTERVAL:*/2 * * * *}`) + skill `investigate-alert` → Telegram. Hermes видит Prometheus-алерты. ADR `docs/adr-hermes-alert-pipeline.md`. CI guard `ci:hermes-agent-smoke` check #8 (5 sub-checks). Tests: hermes-gateway 184/184 ✅, mcp-server 62/62 ✅. | — | — | | ✅ resolved (P7-7) |
...
| **M6** | ~~Миграции не применяются автоматически при deploy (нет one-shot migrator контейнера)~~ → **✅ RESOLVED (2026-08-01, P7-1):** one-shot migrator-контейнер (`infra/docker/Dockerfile.migrator`, лёгкий node:22-alpine + `pg` + `tools/db-migrate.mjs` + миграции) добавлен в `docker-compose.prod.yml` как сервис `migrator` с `depends_on.postgres.service_healthy`. Все 13 БД-связанных сервисов зависят от него через `condition: service_completed_successfully` (через якорь `*nest-common` + явные `depends_on`). `docker compose up -d` применяет миграции автоматически; идемпотентен (пропуск по `schema_migrations`). `docs/deployment-guide.md` §7 обновлён. | — | — | | ✅ resolved (P7-1) |
...
| **M8** | ~~Backup стратегия не автоматизирована (нет pg_dump cron/WAL archiving в compose)~~ → **✅ RESOLVED (2026-08-01, P7-2):** backup-сайдкар (`infra/docker/Dockerfile.backup` + `run-backup.sh` + `entrypoint.backup.sh`) добавлен в `docker-compose.prod.yml` как сервис `backup`. Переиспользует каноничный `tools/backup-postgres.sh` (pg_dump + gzip + retention), cron `BACKUP_SCHEDULE` (по умолч. daily 02:00 UTC), volume `arbibot_backup_data`, optional S3. HEALTHCHECK по маркеру `.last-backup-status` (fail/stale>26h → unhealthy). Initial backup на старте (fail-fast). `docs/disaster-recovery-plan.md` §2.2 обновлён. | — | — | | ✅ resolved (P7-2) |
| **M9** | ~~`node_exporter` отсутствует → `DiskSpaceLow` alert будет без данных~~ → **✅ RESOLVED (2026-08-01, P7-3):** `node_exporter` добавлен в `infra/docker-compose.dev.yml` + `docker-compose.prod.yml` (bind-mount хост-ФС, `--path.rootfs=/host`), scrape job в `prometheus.yml` + `prometheus.dev.yml`. Дополнительно: `DiskSpaceLow` (`alerts.yml`) стал two-tier (warning <15% / **`DiskSpaceCritical` critical <5%**), routing в `alertmanager.yml(.tpl)` расширен (`DiskSpaceLow` в route `infrastructure`); Slack убран из prod-конфигов (operator использует Hermes → Telegram, P7-7), PagerDuty оставлен для critical. Stray-директория `alertmanager.yml;C` удалена. | — | — | | ✅ resolved (P7-3) |
...
| **L3** | ~~`PRIVATE_KEY_ENCRYPTION_KEY` не в validator~~ → **✅ RESOLVED (2026-08-01, P7-6):** добавлен в `tools/validate-env.sh` (вместе с новым `VAULT_MASTER_KEY_SALT`), fail в prod если не задан. | — | — | | ✅ resolved (P7-6) |
...
| Дата | Что |
|------|-----|
| 2026-08-01 | **PLAN7 COMPLETE — P7-pre + P7-3 + P7-7 + P7-1 + P7-6 + P7-2 + P7-4 + P7-5 (8 шагов, все live-blocker'ы закрыты):** P7-pre — code-first verify risk tracker против кода: C1/C3/H2/H3/H4/H5 уже закрыты в коде → ✅ resolved; миграции в `AGENTS.md` 001–047 → **001–049**. **P7-3 (M9):** `node_exporter` + two-tier DiskSpaceLow/Critical + Slack убран, PagerDuty оставлен. **P7-7 (H6 — Hermes alert pipeline → Telegram):** gateway `GET /hermes/v1/alerts` + MCP tool `list_alertmanager_incidents` (25 tools) + cron `alert_watch` + skill `investigate-alert`; ADR `docs/adr-hermes-alert-pipeline.md`; tests hermes-gateway **184/184 ✅**, mcp-server **62/62 ✅**. **P7-1 (M6 — migrator):** `Dockerfile.migrator` + сервис `migrator` в prod compose; 13 БД-сервисов через `service_completed_successfully` → `docker compose up -d` применяет миграции автоматически. **P7-6 (H1+L3 — vault salt):** `VAULT_MASTER_KEY_SALT` env с backward-compatible fallback; `validate-env.sh` fail-closed в prod; ADR `docs/adr-vault-salt.md`; nest-platform **89/89 ✅**. **P7-2 (M8 — backup):** backup-сайдкар (`Dockerfile.backup`) в prod compose — cron `BACKUP_SCHEDULE`, volume `arbibot_backup_data`, optional S3, HEALTHCHECK по `.last-backup-status`. **P7-4 (drill Reconciliation P0):** `npm run drill:2` (`tools/drill-2-reconciliation.mjs`) — inject→detect→idempotency→cleanup; runbook `docs/drill-2-reconciliation.md`. **P7-5 (drill Disaster recovery):** `npm run drill:3` (`tools/drill-3-disaster-recovery.mjs`) — backup→drop+restore test-БД→verify migrations→smoke→RTO/RPO; safety-abort при совпадении dbname; runbook `docs/drill-3-disaster-recovery.md`. **Все 6 инициатив live-blocker (#1–#6 + #18) → done в roadmap-vectors.md.** См. [`.cursor/plans/DEVELOPMENT_PLAN7.md`](../.cursor/plans/DEVELOPMENT_PLAN7.md), [`docs/roadmap-vectors.md`](roadmap-vectors.md). |
| 2026-07-26 | **PAD — paper AutoDrive pipeline** (10 этапов): `AutoDriveWorker` (3 фазы post-promotion: `promoted → draft → (opt-in) active → settled`), `PaperTradesService.settle()` (идемпотентный, P/L + reservation cleanup), миграции **046** (`entry_price`/`exit_price`/`profit_usd`/`settled_at`) + **047** (seed `paper.auto_drive`, `enabled: false` safe-by-default), расширенный outbox-контракт (P/L v1.1), API `/paper/trades/history` + `/stats` + BFF + UI (stats cards + history list), panic-button flip `PAPER_AUTO_DRIVE_ENABLED=false`, 15 unit-тестов (5 settle + 10 worker). Promotion остаётся за оператором (paper→live gate). CI job **`e2e-paper-auto-drive`**. См. [`docs/tz-autodrive-audit.md`](tz-autodrive-audit.md), [`docs/paper-auto-drive-config-keys.md`](paper-auto-drive-config-keys.md). |
...
*Последняя актуализация файла: 2026-08-01 (code-first verify перед PLAN7: C1/C3/H2/H3/H4 помечены ✅ resolved — риски уже закрыты в коде, TODO.md отставал; добавлен **H6** — структурный пробел «два домена инцидентов» (Hermes не видит Prometheus-алерты), закрывается шагом `P7-7-HERMES-ALERTS`; см. [`docs/roadmap-vectors.md`](roadmap-vectors.md) принцип P1 и [`.cursor/plans/DEVELOPMENT_PLAN7.md`](../.cursor/plans/DEVELOPMENT_PLAN7.md) §сверка с кодом). Предыдущая актуализация: 2026-07-17 (pre-deploy risk tracker добавлен; исполнительный план — [`docs/pre-deploy-verification-plan.md`](pre-deploy-verification-plan.md)).*
