...
| 2026-07-31 | `@hono/node-server` | GHSA-frvp-7c67-39w9 (`serve-static` path traversal, MODERATE) | MCP SDK `1.29.0`→`1.30.0` (widened its range to `^1.19.9 \|\| ^2.0.5`) + `overrides: { "@modelcontextprotocol/sdk": { "@hono/node-server": "^2.0.5" } }` → resolved `2.0.12`; fresh resolve (`rm -rf node_modules package-lock.json && npm install`) | (pending) |
