...
- [x] `tools/validate-env.sh` блокирует prod-deploy без auth-config — `validate-env.sh:320-322` вызывает `log_fail` при `ARBIBOT_SERVICE_AUTH_ENABLED != 'true'` → exit 1; проверено: auth-disabled → `FAIL: 1, exit 1`; auth-enabled + valid secret → `PASS, exit 2 (warnings only)`
