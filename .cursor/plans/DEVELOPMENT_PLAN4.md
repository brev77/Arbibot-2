...
3. **Навигация по коду через graphify** (снижение расхода токенов):
   - `npm run graphify:rebuild` — обновить граф, если watch-хук не сработал (AST-only, ~30 сек, 0 токенов LLM)
   - `npm run graphify:query -- "<вопрос>"` — найти community/файлы/граничные узлы шага (single-writer boundaries, callers, import-graph). Использовать для навигации, а не тяжёлых Explore-агентов.
   - После навигации читать **точечно** — только нужные функции/типы, не целые файлы.
4. Реализовать, прогнать `Test Commands` из детали шага
5. Обновить `status` в этом файле (`planned` → `done`)
6. Следующий шаг по Dependency Graph

> **Token-эффективность:** graphify-query стоит ~0 токенов (локальный AST-grep); Explore-агенты — десятки-сотни K токенов. Graphify покрывает навигацию (где код, какие файлы в community, single-writer boundary); Explore-агенты оставлять для семантических задач (понять поведение, спроектировать подход). См. `AGENTS.md` → «graphify».
