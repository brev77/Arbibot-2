...
- **Last major update (2026-05-20, session 32):** DEX-2-1-BRIDGE-ACROSS **in_progress** — `BridgeTransferService` + `AcrossBridgeAdapter` + `BridgeTransferPollingWorker` + bridge timeout detection + `BridgeAdapterFactoryService` integrated into `VenueFactoryService` (bridge key resolution) + bridge-aware execution flow in `LegsService`; **23 suites, 303/303 tests pass**; Build 21/21 ✅, Lint 28/28 ✅; **следующие шаги: DEX-2-1-BRIDGE-STG, DEX-2-2-PLAN**
...
- CI зелёный на GitHub Actions не верифицирован (локально lint 28/28 ✅, build 21/21 ✅, tests 303/303 ✅)
