...
- **Last major update (2026-04-30):** DEX план — **14/35 шагов → `done`** (DEX-1.0 все done: ADR-STRUCTURE, TECH-CHOICE, ABIS/`@arbibot/contracts-eth`, RPC/RpcProviderManager, MIGRATIONS/`033_dex_on_chain.sql`, VAULT/KeyVaultService, WALLET-MGT/WalletManagerService, GAS/GasEstimatorService, POOL-DISCOVERY, RISK-POLICIES, FILTERS, ENV-EXAMPLE; DEX-1.1 — 2/5 done: APPROVE-PATTERN, SLIPPAGE); CI fixes (5 ошибок → green); review-step + skills обновлены под DEX; Build 21/21 green; `FE-SETTINGS-POLICY-WORKSPACE` → `implemented` (awaiting `/review-step`); Phase 5 OpenClaw formal steps → `done`; Phase 4 closed (2026-04-21); **следующий шаг: `DEX-1-1-ADAPTER-UNI2`** (критический)
...
- **Migrations:** 001–033 (в т.ч. **`024_fix_rollback_configuration_function.sql`**, **`025_execution_plan_playbook.sql`**, **`026_watchlist_tier_snapshots.sql`**, **`027_route_scoring_history.sql`**, **`028_paper_drift_route_key.sql`**, **`029_intake_policy_seed.sql`** — defaults `intake.throttling` / `intake.routing.tiers`; **`030_paper_promotion_quality_fields.sql`**, **`031_portfolio_position_close_idempotency.sql`**, **`032_dex_filters_seed.sql`** — DEX opportunity filters seed; **`033_dex_on_chain.sql`** — on-chain transactions, wallet states, DEX pools, approvals); policy scope **`020_policy_configuration_scopes.sql`** (исправленный rollback / совместимость)
...
- CI зелёный на GitHub Actions не верифицирован (локально lint/build 21/21 ✅)
