...
**Проект feature-complete.** Все формальные шаги планов 1–5 + DEX выполнены; фаза **D4 deploy-readiness** (Plan 4) доставлена (20/22 done + D4-B-8 descoped + D4-C-4 blocked); **PLAN7** (8 шагов, все live-blocker'ы инфраструктуры закрыты) и **PLAN8** (5/5 шагов, correctness sweep + live-gate enablement) выполнены; **PLAN9** (single-chain Arbitrum live-readiness, 13 шагов) — кодовые шаги доставлены (осталось P9-12 ops prereq); **PLAN10** (live auto-execution, 10 инициатив #35–#44) — **proposed**, закрывает архитектурный gap между `risk_checked` opportunities и live-execution (сегодня opportunity «зависает» в `risk_checked` — нет worker'а, создающего execution plans). Гибрид: opp-service setup-only saga + EO `LegAutoDriverWorker` + HTTP callback feedback; правки Гермеса Р1-2/Р2-1…6 учтены. Cross-chain (мосты) вынесен в отдельный план (не PLAN10).
...
**Current Focus:**
- **PLAN10 — live auto-execution (single-chain)** (10 инициатив #35–#44): закрывает архитектурный gap `risk_checked → execution` (сегодня opportunity зависает в `risk_checked`, нет автоматического создания plans). Гибрид: P10-1…P10-5 opp-service (config + kill-switch + token-resolver + plan-setup + LiveAutoDriveWorker), P10-EO EO `LegAutoDriverWorker` (драйвит `created` legs, которые сегодня зависают — EO не имеет своего driver'а), P10-FB HTTP callback feedback (НЕ outbox read — race `processed_at`), P10-AMT notional→amountIn fallback в `beginExecution`, P10-6 миграции 053/054, P10-7 panic-scripts, P10-8 targeted tests, P10-9 testnet smoke. Все правки Гермеса (Р1-2/4/5/6, Р2-1…6) учтены после факт-чека. См. [`.cursor/plans/DEVELOPMENT_PLAN10.md`](.cursor/plans/DEVELOPMENT_PLAN10.md).
...
**Last major update (2026-08-03, PLAN9 — Single-chain Arbitrum live-readiness):** 13 шагов `proposed` (P9-1…P9-13), сформирован из глубокого аудита кода 2026-08-03 execution-пути (4 параллельных Explore-агента + прямая grep-верификация). Ключевые находки: (1) broadcast on-chain tx внутри `dataSource.transaction()` → double-spend при crash; (2) `on_chain_transactions` никогда не пишется (0 writers) + `DexOutboxEventsService.emit*` 0 call sites → fill-enrichment всегда `undefined`, reconciliation-детекторы мёртвые; (3) nonce не передаётся в `sendTransaction` → гонки; (4) `tx.wait(1)` без timeout в 10 местах; (5) live slippage-gate сравнивает tolerance с собой же (`50>50` всегда false); (6) approve и swap с разных кошельков; (7) reconciliation `runDetectors` только ручной + нет stuck-plan reaper; (8) settlement post-commit HTTP без resume. Cross-chain bridge-блокеры (неверные адреса Arbitrum Inbox/OP Portal/Stargate placeholder, fake ABI, false-positive L2→L1 completion) → PLAN10. Инициативы #24–#34 → `accepted` в `docs/roadmap-vectors.md`. См. [`.cursor/plans/DEVELOPMENT_PLAN9.md`](.cursor/plans/DEVELOPMENT_PLAN9.md), [`docs/roadmap-vectors.md`](docs/roadmap-vectors.md).
...
- **Plan 7** (live-blocker infrastructure sweep, 8/8): **done** ✅ — миграции в `AGENTS.md` 001–047 → **001–049**; `node_exporter` + two-tier DiskSpaceLow/Critical (P7-3); Hermes alert pipeline → Telegram (P7-7 — gateway `GET /hermes/v1/alerts` + MCP tool `list_alertmanager_incidents`, 25 tools); DB migrator sidecar (P7-1); `VAULT_MASTER_KEY_SALT` per-deploy salt (P7-6); backup sidecar (P7-2); reconciliation drill (P7-4); disaster-recovery drill (P7-5). См. [`.cursor/plans/DEVELOPMENT_PLAN7.md`](.cursor/plans/DEVELOPMENT_PLAN7.md)
...
- **Plan 10** (live auto-execution, single-chain, 10 инициатив #35–#44): **proposed** 🆕 — закрывает архитектурный gap `risk_checked → execution`. Сегодня opportunity зависает в `risk_checked` (терминальное состояние): AutoDriveWorker (opp-service) после risk approved идёт только в paper (`paperEnqueue`), а `POST /execution/plans/multi-leg` + `begin-execution` никто не вызывает автоматически. После `begin-execution` legs остаются в `created` навсегда — EO не имеет своего driver'а (stuck-plan-reaper берёт только `submitting`). Гибрид: opp-service setup-only saga (P10-1 config + P10-2 kill-switch read + P10-3 token-resolver + P10-4 plan-setup + P10-5 LiveAutoDriveWorker) → EO `LegAutoDriverWorker` (P10-EO, драйвит `created`→`sent`→`filled`, sequential, live-only filter, state-check после markSent) → settlement-relay HTTP callback (P10-FB, НЕ outbox read — race общей колонки `processed_at` при пересекающихся allowlist) → opp-service `POST /opportunities/:planId/live-completed`. P10-AMT notional→amountIn fallback в `beginExecution` (costEstimator уже там, не в builder). P10-6 миграции 053 (`live.auto_drive` seed, `enabled:false`) + 054 (`live_execution_plan_id` колонка + partial index для дедуп). P10-7 panic-scripts (LIVE_AUTO_DRIVE_ENABLED + LEG_AUTO_DRIVE_ENABLED flip, recover НЕ восстанавливает). P10-8 targeted tests (crash/concurrency/recovery, parity P9-13). amountIn = Модель #1 pre-quoted (scanner/worker считает sell=buy amountOut; recovery для reverted sell через stuck-plan-reaper→manual). Все правки Гермеса (Р1-2 per-leg в EO но с новым driver'ом; Р1-4 wallet резолвит EO сам; Р1-5/Р2-5 pre-quoted; Р1-6/Р2-6 HTTP callback; Р2-1 state-check; Р2-2 конверсия в beginExecution; Р2-3 sequential; Р2-4 live-only filter) учтены после факт-чека. Инициативы #35–#44 → `proposed` в `docs/roadmap-vectors.md`. См. [`.cursor/plans/DEVELOPMENT_PLAN10.md`](.cursor/plans/DEVELOPMENT_PLAN10.md), [`docs/roadmap-vectors.md`](docs/roadmap-vectors.md)
...
- `npm run db:migrate` — apply SQL migrations under `infra/postgres/migrations/` (001–049)
