...
- **Plan 13** (slippage gate same-decimals, инициатива #49): **done** ✅ — capital-safety фикс `enforcePostQuoteSlippageGate` в `uniswap-v2.adapter.ts:417-420` (общая функция для 5 адаптеров: UniV2/SushiV2/PancakeV2/BiswapV2/UniV3). Баг: ветка «same decimals → direct ratio is exact» сравнивала raw human-units при равных decimals, но одинаковые decimals ≠ одинаковая ценность (CRV 18 dec vs WETH 18 dec → 42.92 CRV vs 0.005467 WETH = 9999 bps, блокировка валидного арба). Решение: удалить ветку, всегда сравнивать в USD через `PriceOracleService.getTokenPriceUsd` (oracle кэш 60s + single-flight покрывают overhead). Production-доказательство: CRV/WETH sell leg (42.92 CRV ≈ $9.87 → 0.005467 WETH ≈ $10.39) — теперь impact 0 bps (was 9999). Build/test: EO 871/871 (55 suites, +2 CRV/WETH кейса). Инициатива #49 → `done` в `docs/roadmap-vectors.md`. См. [`docs/plan-slippage-same-decimals-2026-08-10.md`](docs/plan-slippage-same-decimals-2026-08-10.md), [`docs/roadmap-vectors.md`](docs/roadmap-vectors.md)
