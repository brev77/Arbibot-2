...
**Проект feature-complete.** Все формальные шаги планов 1–5 + DEX выполнены; фаза **D4 deploy-readiness** (Plan 4) доставлена (20/22 done + D4-B-8 descoped + D4-C-4 blocked); **PLAN7** (8 шагов, все live-blocker'ы инфраструктуры закрыты) и **PLAN8** (5/5 шагов, correctness sweep + live-gate enablement) выполнены; **PLAN9** (single-chain Arbitrum live-readiness, 13 шагов) — **в работе**. Глубокий аудит кода 2026-08-03 выявил, что execution-путь к реальной отправке on-chain tx недореализован (broadcast внутри DB-tx, `on_chain_transactions` не пишется, nonce-гонки, reconciliation-детекторы мёртвые, live slippage-gate всегда проходит). Cross-chain (мосты) вынесен в PLAN10.
...
**Current Focus:**
- **PLAN9 — single-chain Arbitrum live-readiness** (13 шагов, инициативы #24–#34): кодовые блокеры execution-пути (P9-1 broadcast idempotency, P9-2 OnChainTx persist, P9-3 nonce lock, P9-4 tx.wait timeout, P9-5 live slippage gate, P9-6 approve/swap wallet, P9-7 recon cron+reaper, P9-8 settlement outbox, P9-9 capital idempotency, P9-10 vault salt assert, P9-11 gas clamp) + ops prereq (P9-12) + single-chain smoke (P9-13). См. [`.cursor/plans/DEVELOPMENT_PLAN9.md`](.cursor/plans/DEVELOPMENT_PLAN9.md).
- Cross-chain (мосты: неверные mainnet-адреса, fake Stargate ABI, false-positive L2→L1 completion, отсутствие prove/finalize) → **PLAN10** (не стартовал, bridge venue keys halt'нуты на весь PLAN9).
