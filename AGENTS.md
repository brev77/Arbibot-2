...
- **Last major update (2026-04-28):** DEX Opportunity Filters System реализован (backend `DexFiltersConfigDto` + preview + metrics, frontend `DexFiltersPanel`, миграция `032_dex_filters_seed.sql`); DEX Code Review выявил **3 критических блокера** (разработка DEX приостановлена до исправления); `FE-SETTINGS-POLICY-WORKSPACE` → `implemented` (awaiting `/review-step`); Phase 5 OpenClaw formal steps → `done` (P5-5-GW/OAPI/OCUI/BRIEF); Phase 4 closed (2026-04-21); review gate `PRIO-P2-PAPERDISC` closed (2026-04-19); Monorepo ESLint green; bus-smoke verification — connection tests successful
...
**DEX Code Review — критические блокеры:**
- 🔴 Blocker 1: `getEncryptedKey` не реализован в WalletManager
...
**Windows — repo path with spaces and Nest apps:** Two issues are easy to mix up. (1) **`nest start` / watch:** Node can fail to preload `source-map-support` when the path is split on spaces (error like `Cannot find module 'C:\\Users\\YourName'` with the path **truncated** at the first space). That was a Nest CLI class of bugs ([`nestjs/nest-cli#2358`](https://github.com/nestjs/nest-cli/issues/2358)); keep **`@nestjs/cli` 11.0.21+** and prefer **local** `npm` scripts, not a global `nest` binary. (2) **No `dist/main.js` after a “successful” build:** check `cwd` (artefacts are under `apps/<service>/dist/`), run **`npm run build -w @arbibot/<name>`** from the repo root, and compare with a direct `npx tsc -p apps/<service>/tsconfig.build.json` if in doubt. **In this monorepo**, Nest app **`build`** / **`start`** / **`start:dev`** use **`tsc -p tsconfig.build.json`**, **`node dist/main.js`**, and a **`concurrently` + `tsc --watch` + `node --watch`** dev loop so the CLI does not need to wrap `node` for normal local runs; **`nest-cli.json`** uses **`"builder": "tsc"`** (and you can still run **`npx nest build`** in a package if you need the CLI). **If problems persist:** use a clone path **without spaces**, a **junction**, or `subst X: "C:\\path\\to\\Arbibot 2"` and work from **`X:\\`** (bypasses path quirks for any tool).
...
- `npm run db:migrate` — apply SQL migrations under `infra/postgres/migrations/` (001–032)
