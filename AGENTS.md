...
1. **architecture-guard-agent** — validates changes against Arbibot 2 system architecture
   ...
   - Checks: service boundaries, single-writer, reservation-first, outbox/inbox, reconciliation, paper/live isolation, operator approval for destructive actions, OpenAPI/AsyncAPI consistency, DEX-specific invariants
...
2. **backend-review-agent** — reviews backend code against Arbibot 2 architecture
   ...
   - Checks: NestJS/Fastify services, OpenAPI/AsyncAPI/schema review, single-writer patterns, reservation-first, outbox/inbox, ExecutionPlan state machine, event envelopes; **DEX-specific backend checks** (ethers.js, RPC, gas, slippage, on-chain entities)
...
3. **frontend-review-agent** — reviews frontend code against Arbibot 2 conventions
   ...
   - Checks: Next.js/React review, operator dashboard PR, App Router, React Query, Zustand, shadcn/ui, TanStack Table, operator safety, RBAC, destructive action flows; **DEX-specific frontend checks** (filters panel, wallet UI, health banners)
...
**Проект feature-complete.** Все формальные шаги планов 1–5 + DEX выполнены; фаза **D4 deploy-readiness** (Plan 4) доставлена (20/22 done + D4-B-8 descoped + D4-C-4 blocked); **PLAN7** (8 шагов, все live-blocker'ы инфраструктуры закрыты) и **PLAN8** (5/5 шагов, correctness sweep + live-gate enablement) выполнены; **PLAN9** (single-chain Arbitrum live-readiness, 13 шагов) — кодовые шаги доставлены (P9-1…P9-11, P9-13; осталось P9-12 ops prereq); **PLAN10** (live auto-execution, 10 инициатив #35–#44) — **код доставлен (11/12 done, P10-9 pending)**: opp-service `LiveAutoDriveWorker` + EO `LegAutoDriverWorker` + HTTP callback feedback закрывают архитектурный gap между `risk_checked` opportunities и live-execution (коммит `787fc38`, safe-by-default `LIVE_AUTO_DRIVE_ENABLED=false`). Cross-chain (мосты) вынесен в отдельный план (не PLAN10).

**Quality Metrics (на коммите `c134b0f`, 2026-08-03):** Build 22/22 ✅ | Lint 29/29 ✅ (0 errors) | Tests execution-orchestrator **801/801 ✅** (46 suites), capital-service **24/24 ✅** | Migrations 001–050 (после PAD — 046 paper_trades settlement, 047 paper.auto_drive seed; **048** execution cost breakdown, **049** dex.limits.minNetProfitUsd seed; **050** paper_capital_reservations unique-constraint hotfix)

**Quality Metrics (на коммите `1a7894b`, 2026-08-10 — PLAN11/12/13):** Migrations **001–054** (добавлены: **051** capital UNIQUE(correlation_id) P9-9, **052** execution_legs `submitting` state P9-1, **053** `live.auto_drive` seed P10-6, **054** `arbitrage_opportunities.live_execution_plan_id` P10-6). Build/lint/test детально по планам: PLAN11 — EO 856/856, audit 22/22, contracts-eth 16/16; PLAN12 — EO 869/869 (55 suites), opp-service 201/201 (14 suites); PLAN13 — EO 876/876 (56 suites). ⚠️ Полный monorepo `npm run build`/`lint`/`test` на HEAD не перезапускался при этом обновлении доки — см. CI или запустите локально для свежего snapshot.

**Current Focus:**
- **PLAN10 — live auto-execution (single-chain)** (10 инициатив #35–#44): **код доставлен (11/12 done), P10-9 pending** ✅ — закрывает архитектурный gap `risk_checked → execution` (opportunity зависала в `risk_checked`, не было автоматического создания plans). Коммит `787fc38` (2026-08-04) доставил P10-1…P10-8: opp-service `LiveAutoDriveWorker` + `PlanSetupOrchestrator` + `TokenResolverService` + `LiveKillSwitchService`; EO `LegAutoDriverWorker` (драйвит `execution_legs` `created`→`sent`→`filled`); settlement-relay HTTP callback (НЕ outbox read — race `processed_at`); миграции 053 (`live.auto_drive` seed, `enabled:false`) + 054 (`arbitrage_opportunities.live_execution_plan_id` + partial index); `tools/panic-button.sh` flips `LIVE_AUTO_DRIVE_ENABLED`+`LEG_AUTO_DRIVE_ENABLED`; 4 spec-файла (leg-auto-driver, live-auto-drive-config, live-auto-drive.worker, plan-setup-orchestrator). ⏸ **Открыто:** P10-9 dedicated live-auto-drive smoke (пока переиспользуется PLAN8 `smoke:live-testnet`); P10-AMT (#42) — DTO поле добавлено, runtime конверсия = Phase 2 (pre-quoted path покрывает MVP). **Safe-by-default:** `LIVE_AUTO_DRIVE_ENABLED=false` + `LEG_AUTO_DRIVE_ENABLED=false`. Все правки Гермеса (Р1-2/4/5/6, Р2-1…6) учтены. См. [`.cursor/plans/DEVELOPMENT_PLAN10.md`](.cursor/plans/DEVELOPMENT_PLAN10.md), [`docs/roadmap-vectors.md`](docs/roadmap-vectors.md).
- **PLAN9 — single-chain Arbitrum live-readiness** (кодовые шаги доставлены, осталось P9-12 ops prereq — wallet/RPC/capital env). См. [`.cursor/plans/DEVELOPMENT_PLAN9.md`](.cursor/plans/DEVELOPMENT_PLAN9.md).
- Cross-chain (мосты: неверные mainnet-адреса, fake Stargate ABI, false-positive L2→L1 completion, отсутствие prove/finalize) → **отдельный план** (не PLAN10; bridge venue keys halt'нуты).
- Product decision: paper-first validation → mainnet minimal capital (single-chain Arbitrum).

**Last major update (2026-08-03, PLAN9 — Single-chain Arbitrum live-readiness):** 13 шагов `proposed` (P9-1…P9-13), сформирован из глубокого аудита кода 2026-08-03 execution-пути (4 параллельных Explore-агента + прямая grep-верификация). Ключевые находки аудита (**⚠️ это блокеры выявленные аудитом 2026-08-03; кодовые шаги P9-1…P9-11 с тех пор доставлены — см. состояние плана ниже, пометки ✅**): (1) broadcast on-chain tx внутри `dataSource.transaction()` → double-spend при crash *(✅ P9-1: broadcast вне DB-tx + new `submitting` state, migration 052)*; (2) `on_chain_transactions` никогда не пишется (0 writers) + `DexOutboxEventsService.emit*` 0 call sites → fill-enrichment всегда `undefined`, reconciliation-детекторы мёртвые *(✅ P9-2: `OnChainTransactionService.persistWithOutcome()` — единственный writer, вызывается из `LegsService.completeMarkSent()` atomic с `submitting→sent`; AGENTS.md «0 writers» ниже в списке миграций — устаревшее описание этого аудита, не текущее состояние)*; (3) nonce не передаётся в `sendTransaction` → гонки *(✅ P9-3: `NonceManagerService.withBroadcastLock()`)*; (4) `tx.wait(1)` без timeout в 10 местах *(✅ P9-4: `waitForConfirmation()` с per-chain timeout; единственный оставшийся raw `tx.wait()` — `native-wrap.ts:122` WETH deposit)*; (5) live slippage-gate сравнивает tolerance с собой же (`50>50` всегда false) *(✅ PLAN13 #49: USD compare через `PriceOracleService`)*; (6) approve и swap с разных кошельков *(✅ P9-6)*; (7) reconciliation `runDetectors` только ручной + нет stuck-plan reaper *(✅ P9-7)*; (8) settlement post-commit HTTP без resume *(✅ P9-8)*. Cross-chain bridge-блокеры (неверные адреса Arbitrum Inbox/OP Portal/Stargate placeholder, fake ABI, false-positive L2→L1 completion) → PLAN10. Инициативы #24–#34 → `accepted` в `docs/roadmap-vectors.md`. См. [`.cursor/plans/DEVELOPMENT_PLAN9.md`](.cursor/plans/DEVELOPMENT_PLAN9.md), [`docs/roadmap-vectors.md`](docs/roadmap-vectors.md).

**Last major update (2026-08-02, PLAN8 — Correctness sweep + live-gate enablement):** 5/5 шагов `done` (P8-1 hermes cron-skills reconciliation, P8-2 live-gate correctness, P8-3 wallet-key import CLI, P8-4 live-testnet smoke script, P8-5 pg_dump docker-fallback). Все инициативы #19–#23 → `done` в `docs/roadmap-vectors.md`. Ключевые решения: удалён мёртвый `dex.live` backend-consumer (0 call sites) + `requireApproval` (не enforced, D4-B-8 descoped) — live-gate = kill-switch (`DexKillSwitchService`) + `DEX_VENUE_ENABLED` env (см. [`docs/adr-live-gate.md`](docs/adr-live-gate.md) §2, §8); Arbitrum Sepolia chain-id fixed `421611`→`421614`; новый CLI `npm run wallet:import` (safe key import, AES-256-GCM, fail-closed address validation); новый `npm run smoke:live-testnet` (DoD Gate 3 capital rehearsal + kill-drill + recon); hermes cron-skills: создано 5 недостающих skill-файлов + нормализованы frontmatter (snake_case канон) + CI guard check #9; `backup-postgres.sh` docker-fallback для paper-хоста. См. [`.cursor/plans/DEVELOPMENT_PLAN8.md`](.cursor/plans/DEVELOPMENT_PLAN8.md), [`docs/roadmap-vectors.md`](docs/roadmap-vectors.md).

**Last major update (2026-07-26, PAD — paper AutoDrive pipeline):** реализован автоматический post-promotion pipeline в `paper-trading-service` (см. [`docs/tz-autodrive-audit.md`](docs/tz-autodrive-audit.md), [`docs/paper-auto-drive-config-keys.md`](docs/paper-auto-drive-config-keys.md)). Worker `AutoDriveWorker` (3 фазы: `promoted → draft → (opt-in) active → settled`) + `PaperTradesService.settle()` (идемпотентный, с P/L и reservation cleanup) + миграция **046** (`entry_price`, `exit_price`, `profit_usd`, `settled_at`) + seed **047** (`paper.auto_drive`, `enabled: false` safe-by-default) + расширенный outbox-контракт (P/L v1.1) + API `/paper/trades/history`, `/stats` + BFF + UI (stats cards + history list с win/loss tinting) + panic-button flip `PAPER_AUTO_DRIVE_ENABLED=false`. Архитектурно: promotion остаётся за оператором (paper→live gate), auto-approve opt-in, kill-switch в config-service. CI job `e2e-paper-auto-drive`. 15 новых unit-тестов (5 settle + 10 worker).
...
- **Plan 7** (live-blocker infrastructure sweep, 8/8): **done** ✅ — миграции в `AGENTS.md` 001–047 → **001–049** (позже расширено до 001–054 миграциями PLAN9 051/052 и PLAN10 053/054 — актуальный диапазон см. в Quality Metrics и блоке Migrations ниже); `node_exporter` + two-tier DiskSpaceLow/Critical (P7-3); Hermes alert pipeline → Telegram (P7-7 — gateway `GET /hermes/v1/alerts` + MCP tool `list_alertmanager_incidents`, 25 tools); DB migrator sidecar (P7-1); `VAULT_MASTER_KEY_SALT` per-deploy salt (P7-6); backup sidecar (P7-2); reconciliation drill (P7-4); disaster-recovery drill (P7-5). См. [`.cursor/plans/DEVELOPMENT_PLAN7.md`](.cursor/plans/DEVELOPMENT_PLAN7.md)
- **Plan 8** (correctness sweep + live-gate enablement, 5/5): **done** ✅ — hermes cron-skills reconciliation (P8-1: 5 новых skill-файлов + CI guard check #9); live-gate correctness (P8-2: удалён мёртвый `dex.live` + `requireApproval`, исправлен Arbitrum Sepolia chain-id `421611`→`421614`); wallet-key import CLI (P8-3: `npm run wallet:import`, AES-256-GCM, fail-closed address validation); live-testnet smoke script (P8-4: `npm run smoke:live-testnet`, DoD Gate 3); pg_dump docker-fallback (P8-5: `backup-postgres.sh` auto-detect). Критерий корректности: ни одна «защита live» не заявлена в docs без enforcement в коде. См. [`.cursor/plans/DEVELOPMENT_PLAN8.md`](.cursor/plans/DEVELOPMENT_PLAN8.md), [`docs/roadmap-vectors.md`](docs/roadmap-vectors.md)
- **Plan 9** (single-chain Arbitrum live-readiness, 13 шагов): **кодовые шаги доставлены, осталось P9-12 ops prereq** 🔧 — сформирован из глубокого аудита кода 2026-08-03 execution-пути. Кодовые блокеры: P9-1 broadcast idempotency (broadcast вне DB-tx + new `submitting` state), P9-2 OnChainTx persist + DexOutbox wiring (0 writers → wire), P9-3 per-wallet nonce lock, P9-4 `tx.wait` timeout + confirmation poller (10 мест), P9-5 live slippage gate (реальный price-impact вместо tolerance `50>50`), P9-6 approve/swap один кошелёк, P9-7 reconciliation cron + stuck-plan reaper, P9-8 settlement outbox relay (at-least-once вместо post-commit HTTP), P9-9 capital UNIQUE(correlation_id) + sweeper (migration 051), P9-10 vault salt runtime assert (fail-closed in prod), P9-11 gas policy clamp (dead code → wire). Ops + smoke: P9-12 (wallet/RPC/capital env), P9-13 (single-chain Arbitrum smoke + crash/concurrency targeted-тесты, CI job `e2e-singlechain-live-readiness`). **Cross-chain (мосты) → отдельный план** (не PLAN10), bridge venue keys halt'нуты. Инициативы #24–#34 → `accepted` в `docs/roadmap-vectors.md`. См. [`.cursor/plans/DEVELOPMENT_PLAN9.md`](.cursor/plans/DEVELOPMENT_PLAN9.md), [`docs/roadmap-vectors.md`](docs/roadmap-vectors.md)
- **Plan 10** (live auto-execution, single-chain, 10 инициатив #35–#44): **код доставлен (11/12 done), P10-9 pending** ✅ — закрывает архитектурный gap `risk_checked → execution`. Коммит `787fc38` (2026-08-04): opp-service `LiveAutoDriveWorker` + `PlanSetupOrchestrator` + `TokenResolverService` + `LiveKillSwitchService`; EO `LegAutoDriverWorker` (драйвит `execution_legs` `created`→`sent`→`filled`); settlement-relay HTTP callback (НЕ outbox read — race общей колонки `processed_at` при пересекающихся allowlist). ⚠️ **Уточнение схемы:** `created` здесь — значение `execution_legs.state` (state machine: `created, submitting, sent, acknowledged, partiallyFilled, filled, rejected, canceled, timedOut, failed`), **НЕ** `execution_plans.state` (которое не имеет значения `created` — допустимые plan states: `planned, reserved, armed, executing, completed, hedged, unwound, failed, canceled`). P10-AMT notional→amountIn: DTO поле добавлено, runtime конверсия = Phase 2 (pre-quoted path покрывает MVP). P10-6 миграции 053 (`live.auto_drive` seed, `enabled:false`) + 054 (`live_execution_plan_id` колонка + partial index для дедуп). P10-7 panic-scripts (LIVE_AUTO_DRIVE_ENABLED + LEG_AUTO_DRIVE_ENABLED flip, recover НЕ восстанавливает). P10-8 targeted tests (4 spec-файла). ⏸ **P10-9** (dedicated live-auto-drive smoke) — не создан, переиспользуется PLAN8 `smoke:live-testnet`. amountIn = Модель #1 pre-quoted (scanner/worker считает sell=buy amountOut; recovery для reverted sell через stuck-plan-reaper→manual). Safe-by-default: `LIVE_AUTO_DRIVE_ENABLED=false` + `LEG_AUTO_DRIVE_ENABLED=false`. Все правки Гермеса (Р1-2 per-leg в EO но с новым driver'ом; Р1-4 wallet резолвит EO сам; Р1-5/Р2-5 pre-quoted; Р1-6/Р2-6 HTTP callback; Р2-1 state-check; Р2-2 конверсия в beginExecution; Р2-3 sequential; Р2-4 live-only filter) учтены после факт-чека. Инициативы #35–#43 → `done`, #42 → `in-progress`, #44 → `proposed` в `docs/roadmap-vectors.md`. См. [`.cursor/plans/DEVELOPMENT_PLAN10.md`](.cursor/plans/DEVELOPMENT_PLAN10.md), [`docs/roadmap-vectors.md`](docs/roadmap-vectors.md)
- **Plan 11** (post-Hermes live-correctness sweep, 3 инициативы #45–#47): **done** ✅ — сформирован из факт-чека анализа Hermes (2026-08-05). Три реальные проблемы: `FUNC-V3-PRICING` (#45, live-blocker — V3-only токены типа MAGIC блокировались на cost gate; решено через канон `v3Price()` в `@arbibot/contracts-eth` + `slot0.sqrtPriceX96` в `DiscoveredPool`), `SEC-RPC-STATIC-NETWORK` (#46, live-blocker — реверс `6bbe45e` был основан на неверном понимании ethers v6; `staticNetwork: true` возвращён, комментарий исправлен), `REL-AUDIT-IDEMPOTENCY-UUID` (#47, paper-check — `@IsUUID('4')` отклоняло все семантические idempotencyKey; заменено на `@IsString()`). 4 пункта Hermes'а исключены как ложные тревоги или уже закрытые (Chainlink fix, pool discovery seeds, cache null, KNOWN_DECIMALS). Build/test: EO 856/856, audit 22/22, contracts-eth 16/16. Инициативы #45–#47 → `done` в `docs/roadmap-vectors.md`. См. [`docs/plan-hermes-live-correctness-2026-08-06.md`](docs/plan-hermes-live-correctness-2026-08-06.md), [`docs/hermes-vs-zcode-review-2026-08-06.md`](docs/hermes-vs-zcode-review-2026-08-06.md), [`docs/roadmap-vectors.md`](docs/roadmap-vectors.md)
- **Plan 12** (amountIn USD oracle, инициатива #48): **done** ✅ — capital-safety фикс `computeAmountIns` в `TokenResolverService` (opportunity-service). Баг: формула `notionalUsd × 10^decimals1` предполагала что token1 = стейблкоин ($1 = 1 единица); для WETH-quoted пар генерировала 50 WETH ≈ $130000 вместо $50 (production: CRV/WETH @ buyPrice=0.000122). Решение: split `resolve()` → `resolveTokens()` (sync) + public `computeAmountIns(tokens, notional, evidence, quoteUsd)` (sync, формула `notionalUsd / quoteUsd × 10^decimals`); USD-цена quote-токена через существующий EO `PriceOracleService` (3-tier: stables→$1, WETH→Chainlink, long-tail→pool-derived). Новые файлы: EO `PriceController` (`GET /execution/price/:chainId/:tokenAddress`) + opp-service `LivePriceClientService` (non-throwing, `signedFetch`, 5s timeout, reuse `EXECUTION_API_BASE`). Fail-closed: oracle null → worker skip'аетopp с metric label `skip_no_price` (никогда не возвращается к баг-формуле). Defense-in-depth: oracle primary + fail-closed страховка. Все правки Гермеса (4 зам.) учтены; #1 (dead-pool блокер — отдельная подсистема, scanner-side фильтр `minPoolLiquidityUsd`) зафиксирован в §8. Build/test: EO 869/869 (55 suites), opp-service 201/201 (14 suites). Инициатива #48 → `done` в `docs/roadmap-vectors.md`. См. [`docs/plan-amountin-usd-oracle-2026-08-10.md`](docs/plan-amountin-usd-oracle-2026-08-10.md), [`docs/roadmap-vectors.md`](docs/roadmap-vectors.md)
- **Plan 13** (slippage gate + gas leak + native wrap, инициативы #49–#51): **done** ✅ — capital-safety фиксы в EO DEX-адаптерах. `#49` `SEC-SLIPPAGE-SAME-DECIMALS`: `enforcePostQuoteSlippageGate` (общая функция для 5 адаптеров: UniV2/SushiV2/PancakeV2/BiswapV2/UniV3) имела ветку «same decimals → direct ratio is exact» — сравнивала raw human-units при равных decimals, но одинаковые decimals ≠ одинаковая ценность (CRV 18 dec vs WETH 18 dec → 42.92 CRV vs 0.005467 WETH = 9999 bps, блокировка валидного арба). Фикс: удалить ветку, всегда USD compare через `PriceOracleService.getTokenPriceUsd`. `#50` `SEC-APPROVE-AFTER-GATE`: во всех 5 адаптерах ERC20 `approve` (broadcast tx, тратит gas) выполнялся **до** slippage gate — каждый gate-blocked своп тратил ~$0.015 на approve (ETH баланс падал). Фикс: переставить quote+gate перед approve (quote = read-only view call, gas-free). `#51` `FUNC-WRAP-NATIVE-BEFORE-SWAP`: кошелёк содержит naked ETH, но DEX router требует WETH (ERC20) — `TRANSFER_FROM_FAILED` на каждом swap где `tokenIn=WETH` (CRV/WETH, MAGIC/WETH). Фикс: новый `ensureWrappedNativeBalance` (`native-wrap.ts`) перед approve во всех 5 адаптерах — wraps ETH→WETH (`deposit()`) если баланс ниже amountIn, idempotent, fail-soft на RPC error. Production-доказательство: CRV/WETH sell leg — impact 0 bps (was 9999), approve не тратится если gate блокирует, WETH оборачивается автоматически. Build/test: EO 876/876 (56 suites, +2 CRV/WETH кейса +5 native-wrap тестов). Инициативы #49–#51 → `done` в `docs/roadmap-vectors.md`. См. [`docs/plan-slippage-same-decimals-2026-08-10.md`](docs/plan-slippage-same-decimals-2026-08-10.md), [`docs/roadmap-vectors.md`](docs/roadmap-vectors.md)
...
- **Migrations:** 001–054 (в т.ч. **`024_fix_rollback_configuration_function.sql`**, **`025_execution_plan_playbook.sql`**, **`026_watchlist_tier_snapshots.sql`**, **`027_route_scoring_history.sql`**, **`028_paper_drift_route_key.sql`**, **`029_intake_policy_seed.sql`** — defaults `intake.throttling` / `intake.routing.tiers`; **`030_paper_promotion_quality_fields.sql`**, **`031_portfolio_position_close_idempotency.sql`**, **`032_dex_filters_seed.sql`** — seeds `dex.filters` config (legacy intake filters; ⚠️ НЕ scanner config); **`033_dex_on_chain.sql`** — on-chain transactions, wallet states, DEX pools, approvals; **`034_on_chain_tx_leg_id_uuid.sql`** — OnChainTransaction.legId bigint→uuid; **`035_dex_live_limits_seed.sql`** — seed `dex.limits` + `dex.live` config; **`036_dex2_crosschain.sql`** — bridge transfers table (`bridge_transfers`) + `execution_legs`/`on_chain_transactions` cross-chain columns (⚠️ `cross_chain_reconciliation` **таблицы не существует** — `CrossChainReconciliationService` in-memory сервис, exposes через `bridge-recon.controller.ts`; reconciliation-таблица общего вида — `reconciliation_mismatches`, migration `011`); **`037_fix_get_effective_config_value.sql`** — fix rollback for effective config function; **`038_alertmanager_incidents.sql`** — alertmanager incident tracking (D4-A-2); **`039_dex_daily_volume.sql`** — DEX per-token daily volume (D4-B-2); **`040_portfolio_positions_notional_usd.sql`** — notional_usd on portfolio positions; **`041_capital_limits_seed.sql`** — seed `capital.limits` config (D4-B-3); **`042_wallet_keys.sql`** — wallet keys persistence in DB (D4-B-4); **`043_bridge_finality.sql`** — finality columns on `bridge_transfers` (D4-B-5); **`044_scanner.sql`** — scanner tables (`scanner_instances` runtime-status, `scanner_findings`); **`045_scanner_config_seed.sql`** — seed `scanner.defaults` (global fallback filters) + `scanner.instances` (по умолчанию пустой `{"instances": []}`, оператор добавляет через `/settings`); **`046_paper_trade_settlement.sql`** — paper_trades settlement columns `entry_price`/`exit_price`/`profit_usd`/`settled_at` + partial indexes (PAD-2); **`047_paper_auto_drive_seed.sql`** — seed `paper.auto_drive` config with `enabled: false` safe-by-default (PAD-4); **`048_execution_cost_breakdown.sql`** — pre-trade cost estimate persistence (plan-gate net-profit audit); **`049_dex_limits_min_net_profit_seed.sql`** — seed `dex.limits.minNetProfitUsd` floor, default $0.50, LIVE-only gate); **`050_paper_capital_reservation_unique_fix.sql`** — **hotfix**: drop over-broad `UNIQUE(instrument_key,state)` на `paper_capital_reservations` (блокировал settle любой сделки, у чьего инструмента уже была `expired`-reservation → зависшие `active` сделки насыщали Phase B concurrency → каскадно вставал весь AutoDrive pipeline), заменён partial unique index `WHERE state='active'`; self-healing (следующий tick Phase C повторяет `expireReservation` на зависших сделках, ручной backfill не нужен); **`051_capital_reservation_correlation_unique.sql`** (P9-9 — UNIQUE(correlation_id) + sweeper); **`052_execution_legs_submitting_state.sql`** (P9-1 — new `submitting` state на `execution_legs`, two-phase mark-sent); **`053_live_auto_drive_seed.sql`** (P10-6 — `live.auto_drive` seed, `enabled: false` safe-by-default); **`054_arbitrage_opportunities_live_plan_id.sql`** (P10-6 — `live_execution_plan_id` column + partial index `WHERE state='risk_checked' AND live_execution_plan_id IS NULL` для дедуп); policy scope **`020_policy_configuration_scopes.sql`** (исправленный rollback / совместимость)
...
- Migration: `032_dex_filters_seed.sql` (seeds `dex.filters` config key — legacy intake filters; ⚠️ **НЕ scanner config** — scanner tables = `044_scanner.sql`, scanner config seed = `045_scanner_config_seed.sql`, keys `scanner.defaults` + `scanner.instances`, последнее по умолчанию пустое `{"instances": []}`)
...
- `CrossChainReconciliationService` — detects and reports cross-chain mismatches **in-memory** (не персистит; `generateBridgeIncident` возвращает structured data для оператора, см. `cross-chain-reconciliation.service.ts:388`)
- `CrossChainReconWorker` — periodic reconciliation cycles
- ~20 tests covering mismatch detection, resolution, and edge cases
- Migration: `036_dex2_crosschain.sql` (создаёт **только** `bridge_transfers` table + cross-chain columns на `execution_legs`/`on_chain_transactions`; ⚠️ `cross_chain_reconciliation` таблицы **не существует** — ранее задокументированная ошибочно. Reconciliation-таблица общего вида — `reconciliation_mismatches`, migration `011`)
...
- **MCP Server:** `packages/hermes-mcp-server/` (`@arbibot/hermes-mcp-server`) — TypeScript MCP server exposing 25 tools via stdio transport → Hermes Gateway HTTP API (14 operational + 1 alertmanager P7-7 + 8 config-management Plan 6 + 2 scanner S4-4)
- **Agent config:** `tools/hermes-agent/` — Hermes Agent (NousResearch) YAML config + MCP connection config
  - `hermes-config.yaml` — LLM provider, messaging (Telegram/Discord), cron, skills path, **MCP server stdio connection** (inline `agent.mcp:` block — `command`, `args`, `env`); это каноничный конфиг, который читает `tools/run-hermes-agent.mjs`
  - `mcp-config.json` — ⚠️ **legacy артефакт Plan 3** (до Plan 5 inline-MCP): файл существует, но `run-hermes-agent.mjs` его НЕ читает — MCP-параметры берутся из `hermes-config.yaml` (`agent.mcp:` block). Оставлен для совместимости с внешними MCP-клиентами (Cursor/Claude), которые ожидают `mcp-config.json` формат; для запуска Hermes Agent — не нужен
...
- **D4 deploy-readiness env vars (added 2026-07-12→16):**
  ...
  - **Paper AutoDrive (PAD-5):** `PAPER_AUTO_DRIVE_ENABLED` (kill-switch, **default `false`** safe-by-default; `tools/panic-button.sh` flips to `false`, `panic-recover.sh` intentionally does NOT restore it), `PAPER_AUTO_DRIVE_INTERVAL_MS`, `PAPER_AUTO_APPROVE`, `PAPER_AUTO_SETTLE_DELAY_MS`, `PAPER_AUTO_DRIVE_MIN_NET_PROFIT_USD`, `PAPER_AUTO_DRIVE_MAX_CONCURRENT_TRADES`, `PAPER_NOTIONAL_USD`, `PAPER_AUTO_DRIVE_BATCH_SIZE` — see [`docs/paper-auto-drive-config-keys.md`](docs/paper-auto-drive-config-keys.md)
  - **Capital ceiling (D4-B-3):** `CAPITAL_MAX_ACTIVE_USD` (aggregate ceiling across reservations + open positions)
  - **Wallet vault (D4-B-4 + P7-6/H1):** `PRIVATE_KEY_ENCRYPTION_KEY` (master AES-256-GCM secret, required), **`VAULT_MASTER_KEY_SALT`** (per-deploy salt for master-key derivation, P7-6 — required in prod, `validate-env.sh` fails closed; backward-compatible fallback for dev); see [`docs/adr-vault-salt.md`](docs/adr-vault-salt.md)
...
**DB migrator (P7-1):** в `infra/docker-compose.prod.yml` сервис `migrator` (`infra/docker/Dockerfile.migrator`) — one-shot контейнер, который запускает `tools/db-migrate.mjs` и выходит с кодом 0. Идемпотентен (пропускает применённые файлы по `schema_migrations`). Каждый Nest-сервис зависит от него через `depends_on.migrator.condition: service_completed_successfully`, поэтому `docker compose up -d` **применяет миграции автоматически** до старта приложений — ручной `npm run db:migrate` в prod больше не нужен (TODO M6 закрыт). Подробнее: [`docs/deployment-guide.md`](docs/deployment-guide.md) §7.

**Automated backup (P7-2):** сервис `backup` (`infra/docker/Dockerfile.backup`) — cron-сайдкар, который переиспользует каноничный `tools/backup-postgres.sh` (pg_dump + gzip + retention) по расписанию `BACKUP_SCHEDULE` (по умолч. `0 2 * * *` = daily 02:00 UTC). Дампы в volume `arbibot_backup_data`, optional S3 (`S3_BACKUP_BUCKET`). HEALTHCHECK по маркеру `.last-backup-status` — тихо отвалившийся cron делает контейнер unhealthy. Initial backup на старте (fail-fast на неверный `DATABASE_URL`). Ручной путь `npm run db:backup` остаётся для dev. TODO M8 закрыт. Подробнее: [`docs/disaster-recovery-plan.md`](docs/disaster-recovery-plan.md) §2.2.
...
- `npm run db:migrate` — apply SQL migrations under `infra/postgres/migrations/` (001–054)
...
- **D4 deploy-readiness scripts:**
  ...
  - `npm run verify:docs` — **foundational-docs freshness guard** (`tools/ci-docs-freshness.sh`); 10 structural checks: migration ranges, skills/tools count, dead markdown links, dead code-block paths, DOCUMENTS_INDEX coverage, plan status vs git, schema table existence, state enum completeness [experimental], cron↔skill mapping (delegates to `ci-hermes-agent-smoke.sh`), ENV var references. ⚠️ **Structural only** — catches numbers/file-existence/links drift; does NOT catch semantic drift (stale qualitative claims like «0 writers»). Allowlist: `tools/docs-freshness-baseline.txt`. CI job `docs-freshness` (path-filtered on-push + weekly cron); pre-commit hook (blocking on Linux/Mac, advisory on Windows).
  ...
  - `npm run e2e:paper-auto-drive` — PAD-8 runtime smoke: drives one promotion candidate end-to-end through `AutoDriveWorker` (promoted → draft → active → settled, asserts P/L + history/stats); needs running `paper-trading-service` with `PAPER_AUTO_DRIVE_ENABLED=true` (`tools/e2e-paper-auto-drive.mjs`)
  - `npm run ci:e2e-paper-auto-drive` — CI wrapper: Postgres + **paper-trading-service** (AutoDrive enabled, short intervals) + `e2e:paper-auto-drive` (`tools/ci-e2e-paper-auto-drive.sh`); GitHub Actions job **`e2e-paper-auto-drive`**
...
- **DEX / Hermes Agent operational scripts:**
  ...
  - `npm run smoke:live-testnet` — **live-testnet smoke** (P8-4, `tools/live-smoke-testnet.mjs`) — end-to-end DoD Gate 3 smoke: capital rehearsal (≤$10, fail-closed) + kill-switch drill (panic:stop → verify `arb_dex_live_halt_active=1` → panic:recover) + reconciliation check (0 mismatches). Dry-run (default) или `--testnet` (real tx). `SMOKE_CAPITAL_USD` (default $1, max $10), `SMOKE_SKIP_KILLDRILL=true` (CI-friendly). See [`docs/live-smoke-runbook.md`](docs/live-smoke-runbook.md), [`docs/live-deploy-dod.md`](docs/live-deploy-dod.md) §Gate 3.
  - `npm run drill:1` — paper-incident operational drill (`tools/drill-1-paper-incident.mjs`)
  - `npm run drill:2` — reconciliation P0 operational drill (`tools/drill-2-reconciliation.mjs`, P7-4) — injects a marked mismatch, verifies detection via `/mismatches`, checks detector idempotency, cleans up; manual operator part (investigating → resolved) measures MTTA/MTTR. See [`docs/drill-2-reconciliation.md`](docs/drill-2-reconciliation.md)
  - `npm run drill:3` — disaster recovery drill (`tools/drill-3-disaster-recovery.mjs`, P7-5) — backup source → drop+recreate+restore test DB → verify migrations → row-count smoke → measured RTO/RPO. Works on a separate `DRILL_TEST_DATABASE_URL` with safety-abort if dbname matches source. See [`docs/drill-3-disaster-recovery.md`](docs/drill-3-disaster-recovery.md)
  - `npm run db:seed-canonical` — seed canonical registry tables (`tools/seed-canonical-registry.mjs`)
  - `npm run wallet:import` — **wallet key import CLI** (P8-3, `tools/wallet-key-import.mjs`) — безопасный ввод приватных ключей в `wallet_keys` для live: читает из stdin/env (НЕ args — не светит в `ps`), валидирует derived address через ethers (fail-closed на mismatch с `--expected-address`), шифрует AES-256-GCM (тот же algorithm что `KeyVaultService`), INSERT в `wallet_keys`. Ключ никогда не логируется. Требует `DATABASE_URL`, `PRIVATE_KEY_ENCRYPTION_KEY`, `VAULT_MASTER_KEY_SALT`. See [`docs/adr-wallet-key-import.md`](docs/adr-wallet-key-import.md), [`docs/vault-integration-guide.md`](docs/vault-integration-guide.md) §6, [`docs/key-rotation-runbook.md`](docs/key-rotation-runbook.md). Usage: `echo "0xKEY" | npm run wallet:import -- --key-id prod-arb-1 --chain-id 42161 --expected-address 0x...`
...
- `@arbibot/hermes-mcp-server` — MCP Server для Hermes Agent (25 tools → gateway: 14 operational + 1 alertmanager P7-7 + 8 config-management Plan 6 + 2 scanner S4-4)
...
- **Paper trades history (PAD-6):** `/api/operator/paper/trades/history` (read-only **GET** → paper `GET /paper/trades/history?from=&to=&limit=`), `/api/operator/paper/trades/stats` (read-only **GET** → paper `GET /paper/trades/stats?from=&to=`)
...
- SQL migrations are applied lexicographically by `tools/db-migrate.mjs`; current migrations **001–054** include: canonical market, market intake idempotency, outbox relay dead-letter fields, execution/portfolio/reconciliation, fill/idempotency, **token/route profiles and risk decision keys** (`015_token_route_profiles.sql`), **paper trading** (`016_paper_trading.sql`, `017_paper_promotion_enqueue_idempotency.sql`), **outbox dedup for `paper-enqueue`** (`018_outbox_paper_enqueue_dedup.sql`), **policy configurations** (`019_policy_configurations.sql`), **policy configuration scopes** (`020_policy_configuration_scopes.sql`, CFG-3), **paper capital reservations** (`021_paper_capital_reservations.sql`), **paper discovery candidates** (`022_paper_discovery_candidates.sql`, `023_paper_discovery_candidates_fixes.sql`), later **`024`–`028`** (execution playbooks, watchlist/scoring history, paper drift `route_key`), **`029_intake_policy_seed.sql`** (defaults for `intake.throttling` / `intake.routing.tiers`), **`030_paper_promotion_quality_fields.sql`**, **`031_portfolio_position_close_idempotency.sql`**, **`032_dex_filters_seed.sql`** (seeds `dex.filters` config — legacy intake filters; ⚠️ НЕ scanner config), **`033_dex_on_chain.sql`** (`on_chain_transactions`, `wallet_states`, `dex_pools`, `approvals` + indexes + triggers), **`034_on_chain_tx_leg_id_uuid.sql`** (OnChainTransaction.legId bigint→uuid), **`035_dex_live_limits_seed.sql`** (seed `dex.limits` + `dex.live` config), **`036_dex2_crosschain.sql`** (`bridge_transfers` table + `execution_legs`/`on_chain_transactions` cross-chain columns; ⚠️ `cross_chain_reconciliation` таблицы **не существует** — `CrossChainReconciliationService` in-memory, exposes через `bridge-recon.controller.ts`), **`037_fix_get_effective_config_value.sql`** (effective config rollback fix), **`038_alertmanager_incidents.sql`** (D4-A-2 paging), **`039_dex_daily_volume.sql`** (D4-B-2 daily-volume), **`040_portfolio_positions_notional_usd.sql`** (notional_usd), **`041_capital_limits_seed.sql`** (D4-B-3 capital ceiling seed), **`042_wallet_keys.sql`** (D4-B-4 wallet keys persistence), **`043_bridge_finality.sql`** (D4-B-5 finality columns), **`044_scanner.sql`** (scanner tables: `scanner_instances` runtime-status, `scanner_findings`), **`045_scanner_config_seed.sql`** (seed `scanner.defaults` + `scanner.instances`, по умолчанию пустой `{"instances": []}`), **`046_paper_trade_settlement.sql`** (PAD-2 paper_trades `entry_price`/`exit_price`/`profit_usd`/`settled_at` + partial indexes), **`047_paper_auto_drive_seed.sql`** (PAD-4 seed `paper.auto_drive` with `enabled: false`), **`048_execution_cost_breakdown.sql`** (pre-trade cost estimate persistence for plan-gate net-profit audit), **`049_dex_limits_min_net_profit_seed.sql`** (seed `dex.limits.minNetProfitUsd` floor, default $0.50, LIVE-only gate), **`050_paper_capital_reservation_unique_fix.sql`** (hotfix: partial unique index `WHERE state='active'`), **`051_capital_reservation_correlation_unique.sql`** (P9-9 — UNIQUE(correlation_id) + sweeper), **`052_execution_legs_submitting_state.sql`** (P9-1 — new `submitting` state на `execution_legs`), **`053_live_auto_drive_seed.sql`** (P10-6 — `live.auto_drive` seed, `enabled: false`), **`054_arbitrage_opportunities_live_plan_id.sql`** (P10-6 — `live_execution_plan_id` column + partial index для дедуп).
...
12. **`e2e-paper-auto-drive`** — after `npm ci` + `npm run build`, runs `npm run ci:e2e-paper-auto-drive` (Postgres + **paper-trading-service** with AutoDrive enabled at short intervals, then `node tools/e2e-paper-auto-drive.mjs`); drives one promotion candidate end-to-end through `AutoDriveWorker` (promoted → draft → active → settled with P/L) and asserts `/paper/trades/history` + `/stats`. See [`docs/paper-auto-drive-config-keys.md`](docs/paper-auto-drive-config-keys.md).
13. **`docs-freshness`** — after `actions/checkout` (no `npm ci` needed — pure bash/grep), runs `bash tools/ci-docs-freshness.sh` (`tools/ci-docs-freshness.sh`); foundational-docs structural drift guard — 10 checks: migration ranges, skills/tools count, dead markdown links, dead code-block paths, DOCUMENTS_INDEX coverage, plan status vs git, schema table existence, state enum completeness [experimental], cron↔skill mapping, ENV var references [advisory]. Separate workflow `.github/workflows/docs-freshness.yml` (path-filtered on-push + PR + weekly cron `17 6 * * 1` + `workflow_dispatch`). **⚠️ Structural-only**: catches numbers/file-existence/links/schema drift; does NOT catch semantic drift (stale qualitative claims like «on_chain_transactions has 0 writers» — use Hermes `docs-audit` skill or human review for those). Allowlist: `tools/docs-freshness-baseline.txt` (historical snapshots, frozen canonical specs, partial-delivery plans). `continue-on-error: true` during 2-week baseline window (added 2026-08-11; remove on/after 2026-08-25 to make blocking). Pre-commit hook (`.githooks/pre-commit`) — blocking on Linux/Mac, advisory on Windows. Added 2026-08-11.
...
- **[`docs/security-accepted-risks.md`](docs/security-accepted-risks.md)** — living record of Dependabot advisories: closure log (sharp, fast-uri, find-my-way, typeorm — fixed 2026-07-23 via `package.json` overrides; `@hono/node-server` — fixed 2026-07-31 via MCP SDK `1.29.0`→`1.30.0` bump + `@hono/node-server@^2.0.5` override) + accepted risks (`brace-expansion <= 5.0.7` — upstream-blocked: `jest@30`/`typeorm@0.3` pin vulnerable `2.x` and `5.x` major-bump breaks their API; dev/build-time only, not internet-reachable). Also documents the npm overrides gotcha (overrides only apply on a fresh resolve — stale `package-lock.json` silently reuses pinned versions).
