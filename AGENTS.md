...
- **Plan 13** (slippage gate + gas leak, инициативы #49–#50): **done** ✅ — capital-safety фиксы в EO DEX-адаптерах. `#49` `SEC-SLIPPAGE-SAME-DECIMALS`: `enforcePostQuoteSlippageGate` (общая функция для 5 адаптеров: UniV2/SushiV2/PancakeV2/BiswapV2/UniV3) имела ветку «same decimals → direct ratio is exact» — сравнивала raw human-units при равных decimals, но одинаковые decimals ≠ одинаковая ценность (CRV 18 dec vs WETH 18 dec → 42.92 CRV vs 0.005467 WETH = 9999 bps, блокировка валидного арба). Фикс: удалить ветку, всегда USD compare через `PriceOracleService.getTokenPriceUsd`. `#50` `SEC-APPROVE-AFTER-GATE`: во всех 5 адаптерах ERC20 `approve` (broadcast tx, тратит gas) выполнялся **до** slippage gate — каждый gate-blocked своп тратил ~$0.015 на approve (ETH баланс падал). Фикс: переставить quote+gate перед approve (quote = read-only view call, gas-free). Production-доказательство: CRV/WETH sell leg — impact 0 bps (was 9999), approve не тратится если gate блокирует. Build/test: EO 871/871 (55 suites, +2 CRV/WETH кейса). Инициативы #49–#50 → `done` в `docs/roadmap-vectors.md`. См. [`docs/plan-slippage-same-decimals-2026-08-10.md`](docs/plan-slippage-same-decimals-2026-08-10.md), [`docs/roadmap-vectors.md`](docs/roadmap-vectors.md)
