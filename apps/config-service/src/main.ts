...
startOpenTelemetryNodeSdkIfConfigured({ serviceName: 'config-service' });
...
async function bootstrap(): Promise<void> {
  ...
  configureArbibotLogger(app, 'config-service');
  await applyArbibotHttpSecurity(app);
  const fastify = app.getHttpAdapter().getInstance();
  fastify.addHook('preHandler', correlationIdPreHandler);
  if (process.env.METRICS_ENABLED !== 'false') {
    installMetricsOnFastify(fastify, { serviceName: 'config-service' });
  }
  app.useGlobalPipes(
    new ValidationPipe({
      whitelist: true,
