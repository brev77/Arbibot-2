...
startOpenTelemetryNodeSdkIfConfigured({ serviceName: 'capital-service' });
...
async function bootstrap(): Promise<void> {
  ...
  configureArbibotLogger(app, 'capital-service');
  await applyArbibotHttpSecurity(app);
  const fastify = app.getHttpAdapter().getInstance();
  fastify.addHook('preHandler', correlationIdPreHandler);
  if (process.env.METRICS_ENABLED !== 'false') {
    installMetricsOnFastify(fastify, { serviceName: 'capital-service' });
  }
  app.useGlobalPipes(
    new ValidationPipe({
      whitelist: true,
