...
export class OpenclawController {
  ...
  /** Placeholder until OpenClaw session registry exists. */
  @Get('sessions')
  sessions(): { items: unknown[]; note: string } {
    return {
      items: [],
      note: 'OpenClaw session registry is not configured in gateway v0; use audit approvals-queue for recent operator actions.',
    };
  }
