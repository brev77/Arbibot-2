...
import type { OpenclawOperatorMutationDto } from './dto/operator-mutation.dto';
...
export class OpenclawMutationService {
  constructor(
    private readonly upstream: OpenclawUpstreamService,
  ...
  async armPlan(
    planId: string,
    dto: OpenclawOperatorMutationDto,
    ...
    await this.auditMutationResult('OPENCLAW_ARM_PLAN', dto, planId, result, {
      correlationId,
    });
  ...
  async enableSafeMode(
    ...
    await this.audit.appendEntry({
      ...
      action: 'OPENCLAW_SAFE_MODE_ENABLE',
  ...
  async disableSafeMode(
    ...
    await this.audit.appendEntry({
      ...
      idempotencyKey: dto.idempotencyKey,
      actor: dto.operatorId,
      action: 'OPENCLAW_SAFE_MODE_DISABLE',
      resourceType: 'safe_mode',
      resourceId: 'global',
      payload: {
        reason: dto.reason ?? null,
  ...
  private async auditMutationResult(
    ...
    await this.audit.appendEntry({
      ...
      resourceType: 'openclaw_mutation',
      resourceId,
      payload: {
