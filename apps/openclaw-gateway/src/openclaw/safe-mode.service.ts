...
const REDIS_KEY = 'arbibot:openclaw:safe-mode:v1';
...
/**
 * Safe mode: Redis when `OPENCLAW_SAFE_MODE_REDIS_URL` or `REDIS_URL` is set
 * (unless `OPENCLAW_SAFE_MODE_USE_MEMORY_ONLY=true`). Otherwise in-process only.
 */
@Injectable()
export class SafeModeService implements OnModuleDestroy {
  ...
  constructor() {
    const memoryOnly = process.env.OPENCLAW_SAFE_MODE_USE_MEMORY_ONLY === 'true';
    const url = (
      process.env.OPENCLAW_SAFE_MODE_REDIS_URL ?? process.env.REDIS_URL ?? ''
    ).trim();
  ...
  private ttlSeconds(): number {
    const raw = process.env.OPENCLAW_SAFE_MODE_REDIS_TTL_SECONDS?.trim();
