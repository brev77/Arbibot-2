...
describe('OpenclawMutationService', () => {
  const makeService = (
    upstream: OpenclawUpstreamService,
    audit?: AuditClientService,
  ) =>
    new OpenclawMutationService(
      upstream,
      audit ??
        ({
  ...
  it('armPlan proxies to execution and audits on success', async () => {
    ...
    const upstream = { postJson } as unknown as OpenclawUpstreamService;
  ...
  it('enableSafeMode updates state and audits', async () => {
    ...
    expect(calls.some((c) => (c[0] as { action?: string }).action === 'OPENCLAW_SAFE_MODE_ENABLE')).toBe(
      true,
    );
  });
});
