 ...
 */
@Injectable()
export class HermesRateLimitService {
  private readonly log = new Logger(HermesRateLimitService.name);
  ...
  private windowMs(): number {
    const raw = process.env.HERMES_MUTATION_RATE_LIMIT_WINDOW_MS;
  ...
  private maxPerWindow(): number {
    const raw = process.env.HERMES_MUTATION_RATE_LIMIT_MAX;
    if (raw === undefined || raw.trim().length === 0) {
      return DEFAULT_MAX;
    }
    const n = Number.parseInt(raw, 10);
  ...
  allow(apiKey: string): boolean {
    if (process.env.HERMES_MUTATION_RATE_LIMIT_ENABLED === 'false') {
      return true;
    }
    const now = Date.now();
    ...
    if (b.count >= max) {
      this.log.warn(`Hermes mutation rate limit exceeded for key hash`);
      return false;
    }
    b.count += 1;
    return true;
