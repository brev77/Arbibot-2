...
startOpenTelemetryNodeSdkIfConfigured({ serviceName: 'scanner-service' });
...
async function bootstrap(): Promise<void> {
  ...
  configureArbibotLogger(app, 'scanner-service');
  await applyArbibotHttpSecurity(app);
  const fastify = app.getHttpAdapter().getInstance();
  fastify.addHook('preHandler', correlationIdPreHandler);
  if (process.env.METRICS_ENABLED !== 'false') {
    installMetricsOnFastify(fastify, { serviceName: 'scanner-service' });
  }
  app.useGlobalPipes(
    new ValidationPipe({
      whitelist: true,
