...
export async function POST(
  ...
  if (!action || !['approve', 'reject', 'cancel'].includes(action)) {
    return NextResponse.json(
      { error: 'Invalid action. Must be one of: approve, reject, cancel' },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const response = await fetch(
      `${process.env.PAPER_API_BASE || 'http://127.0.0.1:3018'}/paper/trades/${encodeURIComponent(id)}/${encodeURIComponent(action)}`,
      {
        method: 'POST',
        headers,
    ...
    return NextResponse.json(
      { error: `Failed to ${action} paper trade` },
      { status: 500 },
    );
  }
}
