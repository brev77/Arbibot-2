...
import type {
  OpenclawApprovalsQueue,
  OpenclawDashboardSummary,
  OpenclawIncidentBriefs,
  OpenclawPlansPage,
  OpenclawPositionsPage,
  OpenclawSafeModeStatus,
  OpenclawSessionsInfo,
  ...
  const close = useMutation({
    mutationFn: () =>
      fetchOperatorBffJson<unknown>(
        `/openclaw/v1/positions/${props.positionId}/close`,
        {
          method: 'POST',
          body: {
            expectedEntityVersion: props.entityVersion,
            approveReason: 'operator_close_via_openclaw',
          },
        },
      ),
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: operatorKeys.openclawPositions });
    },
  });
  ...
  return (
    <tr className="border-b border-border">
      ...
      <td className="px-2 py-1 text-right">
        <OpenclawPlanActions planId={id} state={state} />
      </td>
    </tr>
  );
}

function OpenclawPlanActions(props: {
  ...
  const arm = useMutation({
    mutationFn: () =>
      fetchOperatorBffJson<unknown>(`/openclaw/v1/plans/${props.planId}/arm`, {
        method: 'POST',
        body: {},
      }),
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: operatorKeys.openclawPlans(50) });
    },
  });
  ...
  const execute = useMutation({
    mutationFn: () =>
      fetchOperatorBffJson<unknown>(
        `/openclaw/v1/plans/${props.planId}/execute`,
        { method: 'POST', body: {} },
      ),
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: operatorKeys.openclawPlans(50) });
...
export function OpenclawWorkspace(): ReactNode {
  const qc = useQueryClient();
  const plans = useQuery({
    queryKey: operatorKeys.openclawPlans(50),
    queryFn: () =>
      fetchOperatorBffJson<OpenclawPlansPage>('/openclaw/v1/plans?limit=50'),
  });
  const dashboard = useQuery({
    queryKey: operatorKeys.openclawDashboard,
    queryFn: () =>
      fetchOperatorBffJson<OpenclawDashboardSummary>(
        '/openclaw/v1/dashboard/summary',
      ),
    staleTime: 30_000,
  });
  const briefs = useQuery({
    queryKey: operatorKeys.openclawIncidentBriefs,
    queryFn: () =>
      fetchOperatorBffJson<OpenclawIncidentBriefs>(
        '/openclaw/v1/incident-briefs',
      ),
    staleTime: 20_000,
  });
  const approvals = useQuery({
    queryKey: operatorKeys.openclawApprovalsQueue(40),
    queryFn: () =>
      fetchOperatorBffJson<OpenclawApprovalsQueue>(
        '/openclaw/v1/approvals-queue?limit=40',
      ),
    staleTime: 15_000,
  });
  ...
  const sessions = useQuery({
    queryKey: operatorKeys.openclawSessions,
    queryFn: () =>
      fetchOperatorBffJson<OpenclawSessionsInfo>('/openclaw/v1/sessions'),
  });
  const positions = useQuery({
    queryKey: operatorKeys.openclawPositions,
    queryFn: () =>
      fetchOperatorBffJson<OpenclawPositionsPage>('/openclaw/v1/positions'),
    staleTime: 15_000,
  });

  const enableSafe = useMutation({
    mutationFn: () =>
      fetchOperatorBffJson<unknown>('/openclaw/v1/safe-mode/enable', {
        method: 'POST',
        body: { reason: 'operator_ui' },
      }),
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: operatorKeys.openclawSafeMode });
    },
  });
  const disableSafe = useMutation({
    mutationFn: () =>
      fetchOperatorBffJson<unknown>('/openclaw/v1/safe-mode/disable', {
        method: 'POST',
        body: { reason: 'operator_ui' },
      }),
    onSuccess: async () => {
    ...
    <main className="p-6 md:p-8 max-w-[1200px] mx-auto space-y-8">
      <header>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          ...
          <code className="text-xs">/openclaw/v1</code>). Mutations are
      ...
      {gatewayMissing && (
        <p className="rounded-md border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-sm text-amber-200">
          ...
          <code className="text-xs">OPENCLAW_GATEWAY_URL</code> and{' '}
          <code className="text-xs">OPENCLAW_BFF_API_KEY</code> for{' '}
