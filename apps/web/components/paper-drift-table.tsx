...
import type { PaperDriftSampleItem } from '@/lib/paper-types';

type Props = {
  readonly items: readonly PaperDriftSampleItem[];
};

export function PaperDriftTable({ items }: Props): ReactNode {
  const columns = useMemo<ColumnDef<PaperDriftSampleItem>[]>(
    () => [
      {
      ...
      {
        accessorKey: 'paperMid',
        header: 'Paper mid',
        cell: (ctx) => <span className="text-xs">{String(ctx.getValue<string>())}</span>,
      },
      {
        accessorKey: 'referenceMid',
        header: 'Ref mid',
        cell: (ctx) => <span className="text-xs">{String(ctx.getValue<string>())}</span>,
      },
      {
