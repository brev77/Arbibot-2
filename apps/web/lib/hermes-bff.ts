...
function gatewayBase(): string | null {
  const b = process.env.HERMES_GATEWAY_URL?.trim();
  return b !== undefined && b.length > 0 ? b.replace(/\/$/, '') : null;
}

/** Server-only key used by the web BFF to authenticate to hermes-gateway. */
function bffHermesApiKey(): string | null {
  const k =
    process.env.HERMES_BFF_API_KEY?.trim() ??
    process.env.HERMES_GATEWAY_API_KEY?.trim();
  ...
  return NextResponse.json(
    {
      ...
      hint: 'Set HERMES_GATEWAY_URL and HERMES_BFF_API_KEY (server-side only)',
    },
    { status: 503 },
  );
}
...
/**
 * Forward a GET to `hermes-gateway` (`/hermes/v1/...`) with API key auth.
 * Used by `/api/operator/hermes/v1/*` routes — never call from the browser with a secret.
 */
export async function proxyHermesGatewayGet(
  pathSegments: string[],
  search: string,
  incomingHeaders: Headers,
): Promise<NextResponse> {
  const base = gatewayBase();
  const key = bffHermesApiKey();
  if (base === null || key === null) {
    return notConfigured();
  }

  const suffix = pathSegments.join('/');
  const url = `${base}/hermes/v1/${suffix}${search}`;
...
/**
 * Forward POST/PATCH to `hermes-gateway` with API key and optional operator header.
 */
export async function proxyHermesGatewayWrite(
  method: 'POST' | 'PATCH',
  pathSegments: string[],
  search: string,
