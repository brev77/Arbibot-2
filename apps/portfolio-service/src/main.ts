...
startOpenTelemetryNodeSdkIfConfigured({ serviceName: 'portfolio-service' });
...
async function bootstrap(): Promise<void> {
  ...
  configureArbibotLogger(app, 'portfolio-service');
  await applyArbibotHttpSecurity(app);
  const fastify = app.getHttpAdapter().getInstance();
  fastify.addHook('preHandler', correlationIdPreHandler);
  if (process.env.METRICS_ENABLED !== 'false') {
    installMetricsOnFastify(fastify, { serviceName: 'portfolio-service' });
  }
  app.useGlobalPipes(
    new ValidationPipe({
      whitelist: true,
