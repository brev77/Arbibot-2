...
startOpenTelemetryNodeSdkIfConfigured({ serviceName: 'paper-trading-service' });
...
async function bootstrap(): Promise<void> {
  ...
  configureArbibotLogger(app, 'paper-trading-service');
  await applyArbibotHttpSecurity(app);
  const fastify = app.getHttpAdapter().getInstance();
  fastify.addHook('preHandler', correlationIdPreHandler);
  if (process.env.METRICS_ENABLED !== 'false') {
    installMetricsOnFastify(fastify, { serviceName: 'paper-trading-service' });
  }
  app.useGlobalPipes(
    new ValidationPipe({
      whitelist: true,
